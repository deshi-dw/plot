...
void ui_plot_path_draw(void* data) {
	...
	for(int i = 0; i < path->path->point_count; i++) {
		...
		draw_circle(plot_x(path->path->points[i].x),
		            plot_y(path->path->points[i].y), 3);
		...
		for(int k = 0; k < detail; k++) {
			...
			draw_circle(plot_x(px + part.origin.x), plot_y(py + part.origin.y),
			            2);
		}
	}
}
...
int ui_plot_path_event(void* data, ui_event_t e) {
	...
	else if(e.type == UI_EVENT_MOUSE_HOLD && path->selected >= 0) {
